...
class LignesPenalitePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<LignesPenalitePage> createState() =>
      _LignesPenaliteListPageState();
}

class _LignesPenaliteListPageState
    extends ConsumerState<LignesPenalitePage> {
  // ── Filtres serveur ────────────────────────────────────────────────────
  ...
  // J-1 par défaut : les pénalités du jour sont généralement constatées et
  // consultées le lendemain.
  ...
  @override
  void didUpdateWidget(covariant LignesPenalitePage oldWidget) {
  ...
  // Le scope éventuel (widget.vehiculeId/chauffeurId) reste appliqué.
  void _load() {
  ...
  Future<void> _pickMois() async {
    ...
    if (r != null) {
      setState(() { _moisSelectionne = r.month; _anneeSelectionnee = r.year; });
      _load();
    }
  }
  ...
  Future<void> _openEncaisserDialog(LignePenalite ligne) async {
  ...
  Widget build(BuildContext context) {
    ...
    // Montant par statut sur les lignes chargées, null = total.
...
// ── Barre filtre date (identique recette) ─────────────────────────────────
...
class _SearchAndStatutBar extends StatefulWidget {
  ...
  final StatutLignePenalite? statutSelectionne;
...
class _LignePenaliteCard extends StatelessWidget {
  final LignePenalite ligne;
  final NumberFormat  money;
  final VoidCallback  onTap;
  final VoidCallback? onEncaisser;
  const _LignePenaliteCard({
    required this.ligne,
    required this.money,
    required this.onTap,
    this.onEncaisser,
  ...
  @override
  Widget build(BuildContext context) {
    final sanctionColor = _couleurSanction(ligne.typeSanction);
    final statutColor   = _couleurStatut(ligne.statut);
  ...
  Widget _buildValeurDroite(Color color) {
    ...
    return switch (ligne.typeSanction) {
  ...
  Color _couleurSanction(TypeSanctionLigne t) => switch (t) {
        TypeSanctionLigne.amende         => Colors.red,
        TypeSanctionLigne.buzzer         => Colors.orange,
        TypeSanctionLigne.avertissement  => Colors.amber.shade700,
        TypeSanctionLigne.immobilisation => Colors.purple,
      };

  Color _couleurStatut(StatutLignePenalite s) => switch (s) {
        ...
        child: Column(
          ...
          children: [
            Stack(alignment: Alignment.center, children: [
              ...
              Icon(Icons.gavel_rounded, size: 38, color: Colors.grey.shade300),
            ]),
            ...
            const SizedBox(height: 6),
            Text('Appuyez sur ✨ pour générer les pénalités',
                style:
