...
///
/// L'utilisateur s'est déconnecté volontairement puis reconnecté : son code
/// TMK est toujours sur l'appareil. Plutôt que de lui en faire choisir un
/// nouveau, on le lui redemande — la saisie rouvre le coffre pour y ranger le
/// refresh token tout neuf.
///
...
class _PinResumePageState extends ConsumerState<PinResumePage> {
  ...
  Future<void> _submit() async {
    setState(() => _busy = true);
    final outcome =
        await ref.read(authNotifierProvider.notifier).resumePin(_code);
    if (!mounted) return;
  ...
  Future<void> _confirmForget() async {
    final confirme = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          ...
          'ouverte : pas besoin de ressaisir votre mot de passe.',
        ),
        actions: [
          TextButton(
    ...
    if (confirme ?? false) {
      await ref.read(authNotifierProvider.notifier).restartPinSetup();
    }
  }
