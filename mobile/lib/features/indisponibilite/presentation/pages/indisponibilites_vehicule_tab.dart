...
import 'indisponibilite_vehicule_detail_page.dart';
import 'indisponibilite_vehicule_form_page.dart';

/// Onglet « Véhicules » de la page Indisponibilités : liste (scroll infini) des
/// immobilisations véhicule. N'a pas de Scaffold : il est hébergé par
/// [IndisponibilitesPage].
class IndisponibilitesVehiculeTab extends ConsumerStatefulWidget {
  const IndisponibilitesVehiculeTab({super.key});

  @override
  ConsumerState<IndisponibilitesVehiculeTab> createState() => _State();
}

class _State extends ConsumerState<IndisponibilitesVehiculeTab> {
  ...
  void _onScroll() {
    ...
    if (pos.pixels >= pos.maxScrollExtent - 300) {
      ref.read(indisponibilitesVehiculeListeProvider.notifier).loadMore();
    }
  }

  void _load() {
    final ds = ref.read(indisponibiliteVehiculeDatasourceProvider);
    ref.read(indisponibilitesVehiculeListeProvider.notifier).load((page, size) async {
      try {
        return Right(await ds.getPage(page: page, size: size));
  ...
  Future<void> _openForm([IndisponibiliteVehicule? i]) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => IndisponibiliteVehiculeFormPage(initial: i)),
    );
    if (mounted) {
  ...
  Future<void> _openDetail(IndisponibiliteVehicule i) async {
    await Navigator.push(
      context,
      MaterialPageRoute(
          builder: (_) => IndisponibiliteVehiculeDetailPage(indisponibilite: i)),
    );
    if (mounted) {
      ref.read(indisponibilitesVehiculeListeProvider.notifier).refresh();
    }
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(indisponibilitesVehiculeListeProvider);
...
class _IndispoCard extends ConsumerWidget {
  final IndisponibiliteVehicule indispo;
  ...
  Future<bool> _confirmSuppression(BuildContext context) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: const Text('Supprimer l\'immobilisation ?'),
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.no_transfer_rounded, size: 64, color: Colors.grey.shade300),
          const SizedBox(height: 12),
          const Text('Aucune immobilisation',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
          const SizedBox(height: 4),
          Text('Déclarez l\'immobilisation d\'un véhicule.',
