...
class RecetteFormPage extends ConsumerStatefulWidget {
  final Recette? initial;
  const RecetteFormPage({super.key, this.initial});

  @override
  ConsumerState<RecetteFormPage> createState() => _RecetteFormPageState();
}

class _RecetteFormPageState extends ConsumerState<RecetteFormPage> {
  ...
  void initState() {
    ...
    _montant =
        TextEditingController(text: r?.montant.toStringAsFixed(2) ?? '');
  ...
  Future<void> _save() async {
    ...
    final notifier = ref.read(recetteNotifierProvider.notifier);
    final error = _isEditing
        ? await notifier.updateRecette(widget.initial!.id!, recette)
        : await notifier.createRecette(recette);
    ...
    if (error != null) {
      ...
      _appToast(context, _isEditing ? 'Recette modifiée !' : 'Recette créée !');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppHeader(title: _isEditing ? 'Modifier recette' : 'Nouvelle recette'),
      body: Form(
        key: _formKey,
        child: ListView(
