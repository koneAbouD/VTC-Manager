...
/// un doigt, quand la biométrie est activée — suffisent à la rouvrir. Pas de
/// nouveau code OTP à attendre.
...
class _PinLockPageState extends ConsumerState<PinLockPage> {
  ...
  /// système d'emblée : c'est le geste attendu, autant l'éviter au chauffeur.
  ...
  Future<void> _submit() async {
    setState(() => _busy = true);
    final outcome =
        await ref.read(authControllerProvider.notifier).unlock(_code);
    if (!mounted) return;
    _traiter(outcome);
  }
  ...
  void _traiter(UnlockOutcome outcome) {
    switch (outcome) {
      ...
      // Renoncement (message nul) : rien à dire, le pavé est là.
      case UnlockBiometricsFailed(:final message):
        message == null ? setState(() => _busy = false) : _reject(message);
    }
  }
  ...
  /// « Code TMK oublié ? » : le seul geste qui abandonne le code. Le chauffeur
  ...
  Future<void> _confirmForget() async {
    final confirme = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Votre code sera effacé de cet appareil. Reconnectez-vous avec le '
          'code reçu par WhatsApp, puis choisissez un nouveau code TMK.',
        ),
        actions: [
          TextButton(
    ...
    if (confirme ?? false) {
      await ref.read(authControllerProvider.notifier).forgetPin();
    }
  }
