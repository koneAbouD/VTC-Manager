-- Une contravention annulée s'horodate (annule_le, motif, auteur) : son statut,
-- lui, ne bouge pas — c'est la date qui la retire des lectures postérieures.
-- v_creances_chauffeurs ne filtrait que sur le statut : l'amende retirée restait
-- donc une dette ouverte du chauffeur. Elle pesait sur sa balance âgée, et
-- surtout un arrêté de compte la comptait parmi les créances à compenser : le
-- fonds de cotisation éteignait une amende que plus rien ne justifiait, reprenant
-- au chauffeur de l'argent qu'il ne devait plus.
--
-- La vue est recréée à l'identique, à ce seul filtre près. Les trois autres
-- documents restent exclus par leur statut : recette, cotisation et pénalité
-- passent toutes ANNULEE quand on les annule.
