...
/**
 * Encaisse plusieurs lignes de cotisation d'un même geste de caisse : le chauffeur
 ...
 * sienne, ouverte par {@link CreateEncaissementCotisationUseCase}. Un refus —
 * période comptable clôturée, caisse du jour déjà comptée, montant supérieur au
 * reste dû — ne vaut que pour la ligne visée :
...
public class CreateEncaissementsCotisationLotUseCase {
    ...
    public List<ResultatEncaissementLot> executer(List<MontantParLigne> lignes,
        ...
        for (MontantParLigne ligne : lignes) {
            try {
                EncaissementCotisation encaissement = EncaissementCotisation.builder()
                        .ligneCotisationId(ligne.ligneId())
                ...
                EncaissementCotisation saved =
                        createEncaissementUseCase.executer(ligne.ligneId(), encaissement);
                ...
                log.warn("Encaissement en lot refusé sur la ligne de cotisation {} : {}",
