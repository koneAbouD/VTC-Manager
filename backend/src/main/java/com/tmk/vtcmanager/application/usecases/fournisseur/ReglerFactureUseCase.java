...
/**
 * Règlement, total ou partiel, d'une facture fournisseur.
 *
...
public class ReglerFactureUseCase {
    ...
    @Transactional
    public FactureFournisseur executer(Long factureId, BigDecimal montant, ModePaiement modePaiement,
                                       Long compteTresorerieId, LocalDate datePaiement,
                                       String commentaire) {
        FactureFournisseur facture = factureRepository.findById(factureId)
                .orElseThrow(() -> ResourceNotFoundException.of("Facture fournisseur", factureId));
        ...
        operationRepository.save(OperationFinanciere.builder()
                .reference(sequenceReferenceService.suivante(
                        SequenceReferenceService.Journal.REGLEMENT_FOURNISSEUR, date))
