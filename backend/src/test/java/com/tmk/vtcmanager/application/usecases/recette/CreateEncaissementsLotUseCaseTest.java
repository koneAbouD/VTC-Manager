 ...
 * transaction unique.
 */
@DisplayName("Encaissement de recettes en lot")
class CreateEncaissementsLotUseCaseTest {

    private static final LocalDate LE_JOUR = LocalDate.of(2026, 9, 2);

    private CreateEncaissementUseCase unitaire;
    private CreateEncaissementsLotUseCase useCase;

    @BeforeEach
    void setUp() {
        unitaire = mock(CreateEncaissementUseCase.class);
        useCase = new CreateEncaissementsLotUseCase(unitaire);
    }
    ...
    private void repondEncaissement(long ligneId, long encaissementId) {
        when(unitaire.executer(eq(ligneId), any(Encaissement.class)))
                .thenReturn(Encaissement.builder().id(encaissementId).build());
    }
    ...
    void encaisseChaqueLigneAvecSonMontant() {
        ...
        assertThat(capture.getAllValues())
                .extracting(Encaissement::getMontant)
    ...
    void unRefusNEmportePasLeLot() {
        repondEncaissement(1L, 11L);
        when(unitaire.executer(eq(2L), any(Encaissement.class)))
        ...
        verify(unitaire).executer(eq(3L), any(Encaissement.class));
    ...
    void refusSansMessage() {
        when(unitaire.executer(eq(1L), any(Encaissement.class)))
